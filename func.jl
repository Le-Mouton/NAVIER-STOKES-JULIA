...
function apply_bc!(u, v, p, nx, ny, j_slot_start, j_slot_end, V_jet)
    ...
    u[:, end] .= 0
    v[:, end] .= 0
    # v[j_slot_start:j_slot_end, end] .= -V_jet
    # u[:,end] .= u[:,end-1]
    # v[:,end] .= v[:,end-1]
end
...
# Vorticité ω = ∂v/∂x - ∂u/∂y aux centres des cellules (différences centrées sur uc/vc)
function compute_vorticity!(omega, uc, vc, dx, dy)
    nx, ny = size(omega)
    @inbounds for j in 1:ny
        @inbounds for i in 1:nx
            ip = min(i+1, nx); im = max(i-1, 1)
            jp = min(j+1, ny); jm = max(j-1, 1)
            dvdx = (vc[ip,j] - vc[im,j]) / ((ip - im) * dx)
            dudy = (uc[i,jp] - uc[i,jm]) / ((jp - jm) * dy)
            omega[i,j] = dvdx - dudy
        end
    end
end

function gif_maker(uc, vc, p, T, omega, T_ref, T_jet, xs, ys, dx, dy, n, Xg, Yg, xi, yj, step)
    speed  = sqrt.(uc.^2 .+ vc.^2)
    sp     = @views speed[xi,yj] .+ 1e-10
    Un     = @views uc[xi,yj] ./ sp .* (dx*step*0.5)
    Vn     = @views vc[xi,yj] ./ sp .* (dy*step*0.5)
    sp_lim = (0.0, max(maximum(speed), 1e-2))   # minimum 0.01 : GR plante si clims trop petites
    T_lim  = (0, 100)
    w_lim  = max(maximum(abs, omega), 1e-1)     # minimum 0.1  : idem pour la vorticité

    p1 = Plots.heatmap(xs, ys, speed'; title="‖u‖  t=$n", color=:plasma,
                 clims=sp_lim, aspect_ratio=1, colorbar=true, dpi=72)
    Plots.quiver!(p1, Xg, Yg; quiver=(vec(Un),vec(Vn)), color=:white, linewidth=0.6)

    p2 = Plots.heatmap(xs, ys, T'; title="T (°C)", color=:hot,
                 clims=T_lim, aspect_ratio=1, colorbar=true, dpi=72)

    p3 = Plots.heatmap(xs, ys, omega'; title="ω (vorticité)", color=:RdBu,
                 clims=(-w_lim, w_lim), aspect_ratio=1, colorbar=true, dpi=72)

    Plots.plot(p1, p2, p3; layout=(1,3), size=(1200, 440), dpi=72)
end
...
function streamlines(uc, vc, xs, ys; nseeds=20, nsteps=2000, ds=4e-4)
    ...
    @inline function interp2(f, x, y)
        ...
        v = (1-fx)*(1-fy)*f[i,j] + fx*(1-fy)*f[i+1,j] +
